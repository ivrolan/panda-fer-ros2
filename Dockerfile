...
RUN echo 'export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/home/user/Libraries/libfranka/lib' >> /home/user/.bashrc 
RUN echo 'export CMAKE_PREFIX_PATH=~/Libraries/libfranka/lib/cmake' >> /home/user/.bashrc
...
#     && cd ~/humble_ws && rosdep install -i --from-path src --rosdistro humble -y --skip-keys "ros-humble-libfranka" 
# Suppresss the XDG errors when running GUI apps like RVIZ
RUN mkdir /tmp/${UID}
RUN chown -R user:user /tmp/${UID}

ENV XDG_RUNTIME_DIR=/tmp/${UID}
ENV CMAKE_PREFIX_PATH=~/Libraries/libfranka/lib/cmake
# RUN cd ~/humble_ws \
#     && source ~/.bashrc \
#     && . /opt/ros/humble/setup.sh \
